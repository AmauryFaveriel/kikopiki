...
class Article extends Component {
    ...
    focusBorder = () => {
        let scroll = document.querySelector('.ArticleContent').scrollTop;
        if ((scroll) <= 0) this.props.hideArticle();
        if (scroll > this.position) this.setState({scrollStyle: this.state.downStyle});
        else this.setState({scrollStyle: this.state.upStyle});
